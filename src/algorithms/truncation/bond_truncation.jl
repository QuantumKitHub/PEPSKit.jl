...
Algorithm struct for the alternating least square (ALS) optimization of a bond between two tensors. 
...
* `tol::Float64=1e-9` : ALS converges when bond SVD spectrum change between two iterations is smaller than `tol`.
...
function bond_truncate(
    ...
    for iter in 1:(alg.maxiter)
        ...
        normalize!(s, Inf)
        # fidelity, cost and bond-s change
