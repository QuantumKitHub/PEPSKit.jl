...
"""
Act the first tensor `op` of an OBC-MPO on PEPO tensor `A` and fuse the outgoing MPO string with the virtual space of `A` along `direction`.
"""
function mpo_path_first(A::PEPOTensor, op, direction::Val{D}) where {D}
    _check_pepo_first_physicalspace(A, op)
    direction_index = _mpo_path_direction(D)
    A′ = twistdual(A, 2)
    F = _mpo_path_outgoing_fuser(
        fuser(storagetype(A), domain(A, direction_index)', space(op, 3)),
        direction_index,
    )
    return _mpo_path_first(A′, op, F, direction)
end

"""
Contract a prepared PEPO tensor with the first MPO tensor and its outgoing fuser.
"""
@generated function _mpo_path_first(A, op, F, ::Val{direction}) where {direction}
...
"""
Act the last tensor `op` of an OBC-MPO on PEPO tensor `A` and fuse the incoming MPO string with the virtual space of `A` along `direction`.
"""
function mpo_path_last(A::PEPOTensor, op, direction::Val{D}) where {D}
    ...
    return _mpo_path_last(A′, op, F, direction)
end

"""
Contract a prepared PEPO tensor with the last MPO tensor and its incoming fuser.
"""
@generated function _mpo_path_last(A, op, F, ::Val{direction}) where {direction}
    virtual_label = _mpo_path_virtual_label(direction)
...
"""
Act the middle tensor `op` of an MPO on PEPO tensor `A` and fuse the incoming and outgoing MPO strings with the virtual space of `A` along `directions = (incoming, outgoing)`.
"""
function mpo_path_middle(A::PEPOTensor, op, directions::Val{D}) where {D}
    incoming, outgoing = D
    incoming == outgoing &&
    ...
    return _mpo_path_middle(A′, op, Fin, Fout, directions)
end

"""
Contract a prepared PEPO tensor with a middle MPO tensor and its incoming and outgoing fusers.
"""
@generated function _mpo_path_middle(
        A, op, Fin, Fout, ::Val{directions}
    ) where {directions}
