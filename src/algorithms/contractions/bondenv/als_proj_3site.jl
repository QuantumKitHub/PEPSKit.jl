...
```
"""
function _apply_proj(a2::MPSTensor, p::MPSBondTensor)
    return @tensor a[Da da; D] := a2[Da da; Dap] * p[Dap; D]
end
...
```
"""
function _apply_proj(q::MPSBondTensor, b2::MPSTensor)
    return @tensor b[D db; Db] := q[D; Dqb] * b2[Dqb db; Db]
end
...
```
"""
function _apply_proj(
        q::MPSBondTensor, m2::GenericMPSTensor{S, 4}, u::MPSBondTensor
    ) where {S}
    return @tensor m[-1 -2 -3 -4; -5] :=
        q[-1; 1] * m2[1 -2 -3 -4; 5] * u[5; -5]
end
...
"""
function _apply_proj(
        q::MPSBondTensor, m2::GenericMPSTensor{S, 4}
    ) where {S}
    return @tensor qm[-1 -2 -3 -4; -5] := q[-1; 1] * m2[1 -2 -3 -4; -5]
end
...
"""
function _apply_proj(
        m2::GenericMPSTensor{S, 4}, u::MPSBondTensor
    ) where {S}
    return @tensor mu[-1 -2 -3 -4; -5] := m2[-1 -2 -3 -4; 5] * u[5; -5]
end
