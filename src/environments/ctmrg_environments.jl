...
function _corner_tensor(
        f, ::Type{T}, ::Type{TA}, left_vspace::S, right_vspace::S = left_vspace
   ) where {T, TA <: AbstractArray{T}, S <: ElementarySpace}
    return f(T, left_vspace ← right_vspace)
end

function _edge_tensor(
      f, ::Type{T}, ::Type{TA}, left_vspace::S, pspaces::P, right_vspace::S = left_vspace
   ) where {T, TA <: AbstractArray{T}, S <: ElementarySpace, P <: ProductSpace}
    return f(T, left_vspace ⊗ pspaces, right_vspace)
end

"""
    CTMRGEnv(
    [f=randn, T=ComplexF64, TA=Matrix{ComplexF64},] Ds_north::A, Ds_east::A, chis_north::B, [chis_east::B], [chis_south::B], [chis_west::B]
...
"""
function CTMRGEnv(
        f, ::Type{T}, ::Type{TA}, Ds_north::A, Ds_east::A, chis_north::B, chis_east::B = chis_north,
    ...
    C_type = tensormaptype(st, 1, 1, TA)
    T_type = tensormaptype(st, N + 1, 1, TA)
    ...
    for I in CartesianIndices(Ds_north)
        ...
        edges[NORTH, r, c] = _edge_tensor(
            f, T, TA, chis_north[r, _prev(c, end)], Ds_north[_next(r, end), c], chis_north[r, c]
        )
        edges[EAST, r, c] = _edge_tensor(
            f, T, TA, chis_east[r, c], Ds_east[r, _prev(c, end)], chis_east[_next(r, end), c]
        )
        edges[SOUTH, r, c] = _edge_tensor(
            f, T, TA, chis_south[r, c], Ds_south[_prev(r, end), c], chis_south[r, _prev(c, end)]
        )
        edges[WEST, r, c] = _edge_tensor(
            f, T, TA, chis_west[_next(r, end), c], Ds_west[r, _next(c, end)], chis_west[r, c]
        )

        corners[NORTHWEST, r, c] = _corner_tensor(
            f, T, TA, chis_west[_next(r, end), c], chis_north[r, c]
        )
        corners[NORTHEAST, r, c] = _corner_tensor(
            f, T, TA, chis_north[r, _prev(c, end)], chis_east[_next(r, end), c]
        )
        corners[SOUTHEAST, r, c] = _corner_tensor(
            f, T, TA, chis_east[r, c], chis_south[r, _prev(c, end)]
        )
        corners[SOUTHWEST, r, c] = _corner_tensor(f, T, TA, chis_south[r, c], chis_west[r, c])
    end
...
function CTMRGEnv(D_north::P, args...; kwargs...) where {P <: Union{Matrix{VectorSpace}, VectorSpace}}
    return CTMRGEnv(randn, ComplexF64, Matrix{ComplexF64}, D_north, args...; kwargs...)
end
...
`chis_west[r, c]` corresponds to the north space of the west edge tensor.
"""
function CTMRGEnv(f, ::Type{T}, ::Type{TA}, network::N, virtual_spaces...) where {T, TA, N <: InfiniteSquareNetwork}
    ...
    return CTMRGEnv(f, T, TA, Ds_north, Ds_east, virtual_spaces...)
end
function CTMRGEnv(network::InfiniteSquareNetwork{O}, virtual_spaces...) where {O}
    return CTMRGEnv(randn, scalartype(O), storagetype(O), network, virtual_spaces...)
end
function CTMRGEnv(network::Union{<:InfinitePartitionFunction{T}, <:InfinitePEPS{T}}, virtual_spaces...) where {T}
    return CTMRGEnv(randn, scalartype(T), storagetype(T), network, virtual_spaces...)
end

# allow constructing environments for implicitly defined contractible networks
function CTMRGEnv(f, ::Type{T}, ::Type{TA}, state::Union{InfinitePartitionFunction, InfinitePEPS}, args...) where {T <: Number, TA <: AbstractArray{T}}
    return CTMRGEnv(f, T, TA, InfiniteSquareNetwork(state), args...)
end
