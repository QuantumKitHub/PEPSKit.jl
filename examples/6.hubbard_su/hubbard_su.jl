...
# ╔═╡ 4aff432e-e8a9-441f-8c72-ba3609d99eba
using Random

# ╔═╡ 4f49c108-6b3c-4d6a-a99f-90922ab8e114
using TensorKit, PEPSKit

# ╔═╡ ba26b46c-4ccd-426b-a3c7-c71c5051f390
md"""
# Hubbard model imaginary time evolution using simple update

Once again, we consider the Hubbard model but this time we obtain the ground-state PEPS by imaginary time evoluation. In particular, we'll use the [`SimpleUpdate`](@ref) algorithm. As a reminder, we define the Hubbard model as
...
# ╔═╡ b1bcace2-28ff-4475-acb2-3ef8e9ca7033
Random.seed!(1298351928);

# ╔═╡ 1a979cdf-711e-414b-8566-c23638e760d2
md"""
...
# ╔═╡ 452bf333-e3fd-450f-8f6a-0ef42586a03f
t, U = 1, 6;

# ╔═╡ aa4e6383-999d-4b1d-9fca-a053b7a9debf
md"""
...
# ╔═╡ 59d49b04-4ea8-4bc1-b308-56dbb3b5e3d4
Dbond = 8;

# ╔═╡ cd7499dc-32b3-41ca-a369-a8c7dcce8d88
physical_space = Vect[fℤ₂](0 => 2, 1 => 2);

# ╔═╡ 5e046ae3-747e-40c5-9f35-59ced565de50
virtual_space = Vect[fℤ₂](0 => Dbond / 2, 1 => Dbond / 2);

# ╔═╡ 6224ad78-cf6c-444e-976a-1a0d1bb6c261
Nr, Nc = 2, 2;

# ╔═╡ 156d773c-96c3-4fb2-a111-71eddddfa33b
H = hubbard_model(Float64, Trivial, Trivial, InfiniteSquare(Nr, Nc); t, U, mu=U / 2);

# ╔═╡ f1a13a5d-c7c0-4fd9-8a42-06826d04a406
md"""
The simple update algorithm evolves an infinite PEPS with weights on the virtual bonds, so we here need to intialize an [`InfiniteWeightPEPS`](@ref). By default, the bond weights will be identity. Note that we here use tensors with real `Float64` entries:
"""

# ╔═╡ d0bb6199-6e09-41cb-bc0c-a552aedc5443
wpeps = InfiniteWeightPEPS(rand, Float64, physical_space, virtual_space; unitcell=(Nr, Nc));

# ╔═╡ 51df2d55-e51f-41cf-8fd0-bd58ca85c977
md"""
Before starting the simple update routine, we normalize the vertex tensors:
"""

# ╔═╡ 30053016-35f6-49d9-a574-4f53eb702200
for ind in CartesianIndices(wpeps.vertices)
    wpeps.vertices[ind] /= norm(wpeps.vertices[ind], Inf)
end;

# ╔═╡ ada2d1a4-f7d2-4424-bc57-490556a7c4d1
md"""
...
# ╔═╡ 2e9699be-2d90-46e3-ab47-7f75deddcc22
dts = [1e-2, 1e-3, 4e-4, 1e-4];

# ╔═╡ 444fc05e-28a3-47af-a328-5d332004f465
tols = [1e-6, 1e-8, 1e-8, 1e-8];

# ╔═╡ 76d89b68-5356-490f-96f4-ec2704fdf8ff
maxiter = 20000;
...
# ╔═╡ 671286d1-7eee-413e-8ed2-f3c140ff9999
peps = InfinitePEPS(wpeps_su);

# ╔═╡ c1de88b7-f054-4063-86b6-ae234b4c4b0d
md"""
...
# ╔═╡ 5d2c06fd-a954-4b4b-a442-1322ea5eb5a3
χenv₀, χenv = 6, 20;

# ╔═╡ 5f6f179c-6d44-4316-9e7f-ff25bfc0f95d
...
# ╔═╡ 474d4eb6-3d1a-4816-879f-a50e40f7fe9b
E = expectation_value(peps, H, env) / (Nr * Nc);

# ╔═╡ c439d59e-2643-4532-89f3-3dcf92e0c49f
@show E

# ╔═╡ 5a0064cf-2ad4-4d2c-818f-e90ca9a1b82d
md"""
Finally, we can compare the obtained ground-state energy against the literature, namely the QMC estimates from [Qin et al.](@cite qin_benchmark_2016). We find that the result generally agree:
"""

# ╔═╡ ce6851d5-3cba-4393-9ca3-a5f49c5d0dde
Es_exact = Dict(0 => -1.62, 2 => -0.176, 4 => 0.8603, 6 => -0.6567, 8 => -0.5243);

# ╔═╡ 6a17a167-5a49-40a7-9308-6bc687ce935e
E_exact = Es_exact[U] - U / 2;

# ╔═╡ 36427835-e43f-4458-b600-bbbc75cf22a7
