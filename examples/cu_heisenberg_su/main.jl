...
using TensorKit, Adapt, PEPSKit, MPSKitModels, CUDA, cuTENSOR
...
Nr, Nc = 2, 2
H = adapt(CuArray, real(heisenberg_XYZ(ComplexF64, symm, InfiniteSquare(Nr, Nc); Jx = 1, Jy = 1, Jz = 1)));
...
peps = InfinitePEPS(rand, CuMatrix{Float64}, physical_space, bond_space; unitcell = (Nr, Nc));
wts = SUWeight(peps);
...
env₀ = CTMRGEnv(rand, CuMatrix{Float64}, peps, env_space)
