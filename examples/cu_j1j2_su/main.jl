...
using Random
using TensorKit, Adapt, PEPSKit, Strided, CUDA, cuTENSOR, MatrixAlgebraKit
...
peps = InfinitePEPS(rand, CuMatrix{Float64}, Pspace, Vspace; unitcell = (Nr, Nc));
wts = SUWeight(peps);
...
tols = [1.0e-9, 1.0e-9]
J2 = 0.5
H = adapt(CuArray, real(j1_j2_model(ComplexF64, symm, InfiniteSquare(Nr, Nc); J1, J2, sublattice = false)))
...
env₀ = CTMRGEnv(rand, CuMatrix{Float64, CUDA.DeviceMemory}, peps, Espace)
env, = leading_boundary(env₀, peps; tol = 1.0e-10, alg = :sequential, trunc = trunc_env, decomposition_alg = SVDAdjoint(; fwd_alg = CUSOLVER_QRIteration()));
