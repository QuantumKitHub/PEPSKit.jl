...
using TensorKit, PEPSKit, MPSKit, CUDA, cuTENSOR, MatrixAlgebraKit
...
ψ = InfinitePEPS(randn, CuMatrix{ComplexF64}, ComplexSpace(2), ComplexSpace(2))
...
mps₀ = initialize_mps(T, [ComplexSpace(20)]; alg_orth = CUSOLVER_HouseholderQR(; positive=true))
...
env_ctmrg, = leading_boundary(CTMRGEnv(ψ, ComplexSpace(20)), ψ; tol = 1.0e-6, verbosity = 2, svd_alg = SVDAdjoint(; fwd_alg = CUSOLVER_QRIteration()))
...
ψ_2x2 = InfinitePEPS(rand, CuMatrix{ComplexF64}, ComplexSpace(2), ComplexSpace(2); unitcell = (2, 2))
...
mps₀_2x2 = initialize_mps(T_2x2, fill(ComplexSpace(20), 2, 2); alg_orth = CUSOLVER_HouseholderQR(; positive=true))
...
env_ctmrg_2x2, = leading_boundary(
    CTMRGEnv(ψ_2x2, ComplexSpace(20)), ψ_2x2; tol = 1.0e-6, verbosity = 2, svd_alg = SVDAdjoint(; fwd_alg = CUSOLVER_QRIteration())
)
...
function ising_pepo(β; unitcell = (1, 1, 1))
    ...
    O = TensorMap(CuArray(o), ℂ^2 ⊗ (ℂ^2)' ← ℂ^2 ⊗ ℂ^2 ⊗ (ℂ^2)' ⊗ (ℂ^2)')
...
mps₀_pepo = initialize_mps(transfer_pepo, [ComplexSpace(20)]; alg_orth = CUSOLVER_HouseholderQR(; positive=true))
