...
# ╔═╡ 801e31ee-0207-4ceb-be8b-7326097cd5f8
using Random

# ╔═╡ 0e110dfa-d1fa-4e00-8955-440bc8333f2e
using LinearAlgebra, TensorKit, PEPSKit, QuadGK

# ╔═╡ c48b528a-6334-4639-9d86-7447a62722a7
md"""
...
The idea is to encode the partition function into an infinite square lattice of rank-4 tensors which can then be contracted using CTMRG. These rank-4 tensors are represented by [`InfinitePartitionFunction`](@ref) states, as we will see.

But first, let's seed the RNG and import all required modules:
"""

# ╔═╡ 8294ffc6-6e61-4f00-85d4-0bbf46abc1d1
Random.seed!(234923);

# ╔═╡ e9a89299-420b-4d69-8783-6227e87f3156
md"""
...
# ╔═╡ cef9e388-3f2a-4bc9-9510-f4a8333ea7b8
...
# ╔═╡ 22970727-5426-400d-9fae-37ab85046cc4
md"""
...
# ╔═╡ 43bebcf1-7dde-4c72-9cfa-4d229c53d624
beta = 0.6;

# ╔═╡ f2e44974-de37-4bc4-a4d1-42fa2f144233
O, M, E = classical_ising(; beta);

# ╔═╡ e38a0809-5adc-4b32-895e-83005a12d022
Z = InfinitePartitionFunction(O);

# ╔═╡ 0fb5ff79-89d1-4dc2-8902-cbef6082bd48
md"""
...
# ╔═╡ c14d1429-8ea2-454e-973d-7380385fdb76
χenv = 20;

# ╔═╡ 9957dd70-45e3-4714-8467-bb9f5587056c
env₀ = CTMRGEnv(Z, χenv);

# ╔═╡ 32a3820b-b23f-49e0-9b6f-20be53d4dd14
env, = leading_boundary(env₀, Z; tol=1e-8, maxiter=500);

# ╔═╡ 23761bb7-f167-4c29-b68e-7aa4be83c258
md"""
...
# ╔═╡ a49b029e-e822-40ff-8a0d-6e7d3d38c2e3
space.(env.edges)

# ╔═╡ 5f4e8cc6-dc0a-4018-a2ef-8eda2379121e
md"""
To compute the value of the partition function, we have to contract `Z` with the converged environment using [`network_value`](@ref). Additionally, we will compute the magnetization and energy (per site), again using [`expectation_value`](@ref) but this time also specifying the index in the unit cell, where we want to insert the local tensor:
"""

# ╔═╡ 56a7688d-2219-4cf1-a5e6-de6052be6325
λ = network_value(Z, env);

# ╔═╡ 01905c11-725e-4367-983e-3e505288d787
m = expectation_value(Z, (1, 1) => M, env);

# ╔═╡ a2b54391-1f02-4fcb-a193-f655ac70210c
e = expectation_value(Z, (1, 1) => E, env);

# ╔═╡ 2195b7db-910e-4988-99b6-3daa17d30821
md"""
## Comparing against the exact Onsager solution

In order to assess our results, we will compare against the [exact Onsager solution](https://en.wikipedia.org/wiki/Square_lattice_Ising_model#Exact_solution). To that end, we compute the exact free energy, magnetization and energy per site (where `quadgk` performs the integral from ``0`` to ``\pi/2``):
"""

# ╔═╡ 80946864-1461-482e-b86b-41edbb1f573a
...
# ╔═╡ a330f090-a6de-4ded-9f19-ab290f9266f1
f_exact, m_exact, e_exact = classical_ising_exact(; beta);

# ╔═╡ fc26fcd0-3508-49e1-b804-8dcca4a452e3
md"""
...
# ╔═╡ 6aa5c314-35e8-4200-9e50-875460e59553
@show (-log(λ) / beta - f_exact) / f_exact

# ╔═╡ 77c8e924-526a-4f51-b2df-e160fd6530e7
@show (abs(m) - abs(m_exact)) / abs(m_exact)

# ╔═╡ d8845d3c-1a9d-11f0-27c9-d379139ebccc
...
manifest_format = "2.0"
project_hash = "0f5d8f2e954df1ae29d19d3c3ee4ca9cffdd18ac"
