...
using Random
using TensorKit, Adapt, PEPSKit, MPSKit, CUDA, cuTENSOR, MatrixAlgebraKit
...
U = 6
Nr, Nc = 2, 2
H = adapt(CuArray, hubbard_model(Float64, Trivial, Trivial, InfiniteSquare(Nr, Nc); t, U, mu = U / 2));
...
peps = InfinitePEPS(rand, CuMatrix{Float64}, physical_space, virtual_space; unitcell = (Nr, Nc));
wts = SUWeight(peps);
