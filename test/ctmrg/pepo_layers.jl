...
@tensor O2[-1 -2; -3 -4 -5 -6] := O[-1 1; 2 4 6 8] * O[1 -2; 3 5 7 9] * fuser[2 3; -3] * fuser[4 5; -4] * fuser_conj[6 7; -5] * fuser_conj[8 9; -6]
@tensor MO[-1 -2; -3 -4 -5 -6] := M[-1 1; 2 4 6 8] * O[1 -2; 3 5 7 9] * fuser[2 3; -3] * fuser[4 5; -4] * fuser_conj[6 7; -5] * fuser_conj[8 9; -6]
...
# )
#     env, = leading_boundary(CTMRGEnv(network, χenv), network; alg, maxiter=150, projector_alg)
#     env_fused, = leading_boundary(CTMRGEnv(network_fused, χenv), network_fused; alg, maxiter=150, projector_alg)
...
@tensor Oψ[-1; -3 -4 -5 -6] := O[-1 1; 2 4 6 8] * ψ[1; 3 5 7 9] * fuser[2 3; -3] * fuser[4 5; -4] * fuser_conj[6 7; -5] * fuser_conj[8 9; -6]
@tensor Mψ[-1; -3 -4 -5 -6] := M[-1 1; 2 4 6 8] * ψ[1; 3 5 7 9] * fuser[2 3; -3] * fuser[4 5; -4] * fuser_conj[6 7; -5] * fuser_conj[8 9; -6]
...
@testset "PEPO layers CTMRG contraction with dagger using $alg with $projector_alg" for (
    ...
    env, = leading_boundary(CTMRGEnv(network_O, χenv), network_O; alg, maxiter = 200, projector_alg)
    env_fused, = leading_boundary(CTMRGEnv(network_fused_OO, χenv), network_fused_OO; alg, maxiter = 200, projector_alg)
