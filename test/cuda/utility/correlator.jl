...
@testset "Correlator in InfinitePEPS ($(sym))" for sym in syms
    ...
    for Vp in [Vphy]
        op = adapt(CuArray, randn(ComplexF64, Vp ⊗ Vp → Vp ⊗ Vp))
        Pspaces = fill(Vp, size(Nspaces))
        peps = adapt(CuArray, InfinitePEPS(randn, ComplexF64, Pspaces, Nspaces, Espaces))
...
@testset "Correlator in purified InfinitePEPO ($(sym))" for sym in syms
    ...
    for Vp in [Vphy]
        op = adapt(CuArray, randn(ComplexF64, Vp ⊗ Vp → Vp ⊗ Vp))
        Pspaces = fill(Vp, size(Nspaces))
        pepo = adapt(CuArray, InfinitePEPO(randn, ComplexF64, Pspaces, Nspaces, Espaces))
