...
@testset "SimpleUpdate timestep" begin
    Nr, Nc = 2, 2
    H = adapt(CuArray, real(heisenberg_XYZ(ComplexF64, Trivial, InfiniteSquare(Nr, Nc); Jx = 1, Jy = 1, Jz = 1)))
    Pspace, Vspace = ℂ^2, ℂ^4
    ψ0 = adapt(CuArray, InfinitePEPS(rand, Float64, Pspace, Vspace; unitcell = (Nr, Nc)))
    env0 = adapt(CuArray, SUWeight(ψ0))
