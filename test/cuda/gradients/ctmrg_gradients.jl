...
models = [
    adapt(CuArray, heisenberg_XYZ(InfiniteSquare())),
    adapt(CuArray, pwave_superconductor(InfiniteSquare())),
]
...
# the gradient solvers are device-independent and are covered exhaustively by the CPU test,
# so only keep the two KrylovKit code paths here, since GPU gradients are slow
gradient_solver_algs = [[:Arnoldi, :GMRES], [:Arnoldi, :GMRES]]
...
@testset "AD CTMRG energy gradients for $(names[i]) model" verbose = true for i in
    ...
    @testset "ctmrg_alg=:$ctmrg_alg, projector_alg=:$projector_alg, svd_rrule_alg=:$svd_rrule_alg, gradient_alg=(; alg = :$gradient_alg, solver_alg = (; alg = :$gradient_solver_alg))" for (
        ...
        dir = adapt(CuArray, InfinitePEPS(Pspace, Vspace))
        psi = adapt(CuArray, InfinitePEPS(Pspace, Vspace))
        @test storagetype(psi) <: CuArray
...
@testset "AD CTMRG energy gradient accuracy regression test (#276)" begin
    ...
    # initialize randomly
    H = adapt(CuArray, heisenberg_XYZ(InfiniteSquare(1, 1)))
    peps = adapt(CuArray, PEPSKit.peps_normalize(InfinitePEPS(randn, ComplexF64, physicalspace(H)[1], ComplexSpace(3))))
