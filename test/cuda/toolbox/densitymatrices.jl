...
@testset "Single-layer densitymatrix contractions ($I)" for I in keys(ds)
    ...
    D = Ds[I]
    χ = χs[I]
    ρ = adapt(CuArray, InfinitePEPO(d, D; unitcell = (2, 2, 1)))

    ρ_pf = @constinferred InfinitePartitionFunction(ρ)
    env = CTMRGEnv(adapt(CuArray, ρ_pf), χ)

    O = adapt(CuArray, rand(d, d))
    @plansor O_pf[W S; N E] := O[p'; p] * ρ[1, 1, 1][p p'; N E S W]

    # Single site
    O_singlesite = adapt(CuArray, LocalOperator(physicalspace(ρ), ((1, 1),) => O))
    ...
    for inds in zip(
        ...
        O_twosite = adapt(CuArray, LocalOperator(physicalspace(ρ), inds => O ⊗ O))
...
@testset "Double-layer densitymatrix contractions ($I)" for I in keys(ds)
    ...
    env = CTMRGEnv(adapt(CuArray, ρ_peps), χ)

    O = adapt(CuArray, rand(d, d))
    F = adapt(CuArray, isomorphism(fuse(d ⊗ d'), d ⊗ d'))
    @tensor O_doubled[-1; -2] := F[-1; 1 2] * O[1; 3] * twist(F', 2)[3 2; -2]

    # Single site
    site = (1, 1)
    O_singlesite = adapt(CuArray, LocalOperator(physicalspace(ρ), (site,) => O))
    E1 = expectation_value(ρ, O_singlesite, ρ, env)
    O_doubled_singlesite = adapt(CuArray, LocalOperator(physicalspace(ρ_peps), (site,) => O_doubled))
    ...
    for inds in zip(
        ...
        O_doubled_twosite = adapt(CuArray, LocalOperator(physicalspace(ρ_peps), inds => O_doubled ⊗ O_doubled))
