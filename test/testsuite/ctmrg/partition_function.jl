...
function ctmrg_partition_function_spaces(AT)
    return @testset "Check spaces in partition function CTMRG ($AT)" begin
        zA = adapt(AT, randn(ℂ^6 ⊗ ℂ^8 ← ℂ^4 ⊗ ℂ^2))
        zB = adapt(AT, randn(ℂ^2 ⊗ ℂ^9 ← ℂ^5 ⊗ ℂ^6))
        zC = adapt(AT, randn(ℂ^7 ⊗ ℂ^4 ← ℂ^8 ⊗ ℂ^3))
        zD = adapt(AT, randn(ℂ^3 ⊗ ℂ^5 ← ℂ^9 ⊗ ℂ^7))
...
"""
    two_dimensional_classical_ising(AT; beta=log(1 + sqrt(2)) / 2)
...
with storage type `AT`.
"""
function two_dimensional_classical_ising(AT; beta = log(1 + sqrt(2)) / 2, J = 1.0)
    K = beta * J
    ...
    return adapt(AT, TensorMap(o, TMS)), adapt(AT, TensorMap(m, TMS)), adapt(AT, TensorMap(e, TMS))
end
