...
function ctmrg_initialization_critical_ising(AT)
    return @testset "CTMRG environment initialization for critical ising with $S symmetry (#255) ($AT)" for S in symmetries
        # initialize
        Random.seed!(sd)
        T = classical_ising(S)
        O = T[1]
        n = adapt(AT, InfinitePartitionFunction([O O; O O]))
        Venv = make_space(S, χ)
        P = space(O, 2)
...
function ctmrg_initialization_peps(AT)
    return @testset "CTMRG environment initialization for PEPS with $S symmetry ($AT)" for S in symmetries
        # initialize
        Random.seed!(sd)
        P = make_space(S, d)
        Vpeps = make_space(S, D)
        Venv = make_space(S, χ)
        peps = adapt(AT, InfinitePEPS(P, Vpeps; unitcell = (2, 2)))
