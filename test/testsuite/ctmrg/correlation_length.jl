...
function ctmrg_correlation_length(AT)
    return @testset "Correlation length ($AT)" begin
        # regression test for https://github.com/QuantumKitHub/PEPSKit.jl/issues/197
        vD = Vect[SU2Irrep](1 // 2 => 1)
        vd = Vect[SU2Irrep](2 => 1)
        tAKLT_A = adapt(AT, ones(vd ← vD ⊗ vD ⊗ vD ⊗ vD))
        tAKLT_B = permute(adjoint(tAKLT_A), ((5,), (1, 2, 3, 4)))

        ψ = InfinitePEPS([tAKLT_A tAKLT_B; tAKLT_B tAKLT_A]) # this has eltype AbstractTensorMap with no specific storage type
        trunc = trunctol(; atol = 1.0e-12) & truncrank(48)
        boundary_alg = (; tol = 1.0e-10, trunc, maxiter = 400, verbosity = 3)
        env0 = adapt(AT, CTMRGEnv(randn, Float64, ψ, oneunit(vD)))
