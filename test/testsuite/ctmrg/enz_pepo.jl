...
function three_dimensional_classical_ising(AT; beta, J = 1.0)
    K = beta * J
    # Boltzmann weights
...
function enzyme_ctmrg_pepo_runthroughs(AT)
    return @testset "PEPO CTMRG runthroughs for unitcell=$(unitcell) ($AT)" for unitcell in
        ...
        # contract
        T = adapt(AT, InfinitePEPO(O; unitcell = unitcell))
...
function enzyme_ctmrg_pepo_fixed_point(AT)
    return @testset "Fixed-point computation for 3D classical ising model ($AT)" begin
        Random.seed!(81812781144)
        ...
        O, M, E = three_dimensional_classical_ising(AT; beta)
        # contract
        T = adapt(AT, InfinitePEPO(O; unitcell = (1, 1, 1)))
        psi0 = initializePEPS(T, χpeps)
