...
function ctmrg_fixed_iterscheme_asymmetric(AT)
    ...
    return @testset "$unitcell unit cell with $(decomposition_alg.alg) and $projector_alg ($AT)" for (
        ...
        env_conv2, = @testinferred ctmrg_iteration(n, env_conv1, ctm_alg)
        ...
        env_fixed2 = @testinferred gauge_fixed_iteration(env_conv1)
...
function ctmrg_fixed_iterscheme_c4v(AT)
    return @testset "$(decomposition_alg.alg) and $projector_alg ($AT)" for
        ...
        psi = adapt(AT, InfinitePEPS(ComplexSpace(2), ComplexSpace(D)))
        ...
        env_conv2, info = @testinferred ctmrg_iteration(n, env_conv1, ctm_alg) # CHECK
...
function ctmrg_fixed_iterscheme_divide_and_conquer(AT)
    return @testset "Element-wise consistency of :DivideAndConquer and :GKL ($AT)" begin
        ...
        env_conv2_iter, info_iter = @testinferred ctmrg_iteration(n, env_conv1, ctm_alg_iter)
        ...
        env_conv2_full, info_full = @testinferred ctmrg_iteration(n, env_conv1, ctm_alg_full)
        ...
        # do gauge-fixed iteration
        env_fixedsvd_iter = @testinferred gauge_fixed_iteration_iter(env_conv1)
        @test calc_elementwise_convergence(env_conv1, env_fixedsvd_iter) ≈ 0 atol = atol  # This doesn't work for x₀ = rand(size(b, 1))?

        env_fixedsvd_full = @testinferred gauge_fixed_iteration_full(env_conv1)
