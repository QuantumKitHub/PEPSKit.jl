...
function bondenv_truncate(AT)
    return @testset "Bondenv truncation ($AT)" begin
        Random.seed!(0)
        maxiter = 600
        check_interval = 30
        elt = ComplexF64
        ...
        for Vl in (Vqro, Vqro'), Vr in (Vqro, Vqro')
            ...
            Z = adapt(AT, randn(elt, Vext ← Vbond))
            normalize!(Z, Inf)
            benv = Z' * Z
            @info "Dimension of benv = $(Dext)"
            # untruncated bond tensors
            a2 = adapt(AT, randn(elt, Vl ⊗ Vphy ← Vint))
            b2 = adapt(AT, randn(elt, Vint ⊗ Vphy ← Vr'))
