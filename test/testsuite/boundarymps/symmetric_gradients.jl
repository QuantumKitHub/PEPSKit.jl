...
## Tests
# ------
function boundary_mps_symmetric_gradients(AT)
    return @testset "Symmetric boundary MPS energy gradients for $(names[i]) model ($AT)" verbose = true for i in
        eachindex(
            models
        )
        H = models[i]
        ...
        Random.seed!(sd)
        psi = adapt(AT, symmetrize!(InfinitePEPS(Pspace, Vspace), symmetry))
        dir = adapt(AT, symmetrize!(InfinitePEPS(Pspace, Vspace), symmetry))
