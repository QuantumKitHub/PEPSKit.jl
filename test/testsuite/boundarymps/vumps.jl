...
function boundary_mps_one_one_peps(AT)
    ...
    return @testset "(1, 1) PEPS ($AT)" begin
        ...
        mps2, = changebonds(mps, T, OptimalExpand(; trscheme = truncrank(30))) # TODO: update `trunc` to `trunc` once MPSKit does
...
function boundary_mps_fermionic_peps(AT)
    ...
    return @testset "Fermionic PEPS ($AT)" begin
        ...
        n´ = adapt(AT, InfiniteSquareNetwork(map(PEPSKit.mpotensor, PEPSKit.unitcell(n))))
        T´ = adapt(AT, InfiniteMPO(map(PEPSKit.mpotensor, T.O)))
        foreach(V -> (@test V == fuse(D, D')), physicalspace(T´))

        mps´ = adapt(AT, InfiniteMPS(randn, ComplexF64, [physicalspace(T´, 1)], [χ]))
        mps´, env´, ϵ = leading_boundary(mps´, T´, vumps_alg)
        N_vumps´ = abs(prod(expectation_value(mps´, T´)))
...
function boundary_mps_pepo_runthrough(AT)
    ...
    return @testset "PEPO runthrough($AT)" begin
        function ising_pepo(beta; unitcell = (1, 1, 1))
            ...
            return adapt(AT, InfinitePEPO(O; unitcell))
        end
