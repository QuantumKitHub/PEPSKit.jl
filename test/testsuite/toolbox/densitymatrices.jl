...
function toolbox_single_layer_densitymatrix(AT)
    return @testset "Single-layer densitymatrix contractions ($I) ($AT)" for I in keys(ds)
        d = ds[I]
        D = Ds[I]
        χ = χs[I]
        ρ = adapt(AT, InfinitePEPO(d, D; unitcell = (2, 2, 1)))

        ρ_pf = @testinferred InfinitePartitionFunction(ρ)
        env = CTMRGEnv(ρ_pf, χ)
...
function toolbox_double_layer_densitymatrix(AT)
    return @testset "Double-layer densitymatrix contractions ($I) ($AT)" for I in keys(ds)
        d = ds[I]
        D = Ds[I]
        χ = χs[I]
        ρ = adapt(AT, InfinitePEPO(d, D; unitcell = (2, 2, 1)))

        ρ_peps = @testinferred InfinitePEPS(ρ)
        env = CTMRGEnv(ρ_peps, χ)

        O = adapt(AT, rand(d, d))
        F = adapt(AT, isomorphism(fuse(d ⊗ d'), d ⊗ d'))
