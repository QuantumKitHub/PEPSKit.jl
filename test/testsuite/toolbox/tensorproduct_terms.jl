...
function toolbox_tensorproduct_ising(AT)
    return @testset "Tensor product terms reproduce the two-site Ising Hamiltonian ($uc) ($AT)" for uc in
        unitcells
        Random.seed!(2985721)
        ...
        @test expectation_value(peps, H_prod, env) ≈ expectation_value(peps, H, env) rtol =
            1.0e-9

        # scaling a product term scales the term, not each of its factors
        α = 0.37
        @test expectation_value(peps, α * H_prod, env) ≈
            α * expectation_value(peps, H_prod, env) rtol = 1.0e-9
    end
end

function toolbox_tensorproduct_bookkeeping(AT)
    return @testset "Tensor product term bookkeeping ($AT)" begin
