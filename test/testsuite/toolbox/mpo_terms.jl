...
const X = SO.σˣ(ComplexF64, Trivial)
const Z = SO.σᶻ(ComplexF64, Trivial)
...
function toolbox_mpo_convention(AT)
    return @testset "MPO factors follow the standard convention ($AT)" begin
...
function toolbox_mpo_dispatch(AT)
    return @testset "Dispatch separates products from MPOs ($AT)" begin
...
function toolbox_mpo_terms_dense(AT)
    return @testset "MPO terms reproduce dense terms ($(name)) ($AT)" for (name, inds, O) in
        dense_terms
        Random.seed!(2985721)
...
function toolbox_mpo_heisenberg(AT)
    return @testset "MPO terms reproduce the Heisenberg XYZ Hamiltonian ($AT)" begin
        Random.seed!(2985721)
...
function toolbox_mpo_bookkeeping(AT)
    return @testset "MPO term bookkeeping ($AT)" begin
