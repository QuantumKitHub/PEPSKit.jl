...
function timeevol_cluster_bond_truncation(AT)
    return @testset "Cluster bond truncation with projectors ($AT)" begin
        Random.seed!(0)
        N, n = 5, 2
        for (Vphy, Vns, V) in Vspaces
            ...
            Ms1 = map(1:N) do i
                ...
                return adapt(AT, rand(Vw ⊗ Vphy ⊗ Vns' ⊗ Vns ← Ve))
            end
            normalize!.(Ms1, Inf)
...
function timeevol_cluster_identity_gate(AT)
    return @testset "Identity gate on 3-site cluster ($AT)" begin
        N, n = 3, 1
        for (Vphy, Vns, V) in Vspaces
            Vvirs = fill(Vns, N + 1)
            Vvirs[n + 1] = V
            Ms1 = map(1:N) do i
                Vw, Ve = Vvirs[i], Vvirs[i + 1]
                return normalize(adapt(AT, rand(Vw ⊗ Vphy ⊗ Vns' ⊗ Vns ← Ve)), Inf)
            end
            flips = [isdual(space(M, 1)) for M in Iterators.drop(Ms1, 1)]
            unit = adapt(AT, id(Vphy))
        ...
        for (Vphy, Vns, V) in Vspaces
            ...
            Ms1 = map(1:N) do i
                Vw, Ve = Vvirs[i], Vvirs[i + 1]
                return normalize(adapt(AT, rand(Vw ⊗ Vphy ⊗ Vphy' ⊗ Vns' ⊗ Vns ← Ve)), Inf)
            end
            flips = [isdual(space(M, 1)) for M in Iterators.drop(Ms1, 1)]
            unit = adapt(AT, id(Vphy))
            gate = reduce(⊗, fill(unit, 3))
...
function timeevol_cluster_hubbard(AT)
    return @testset "Hubbard model SU (MPO gate) ($AT)" begin
        Nr, Nc = 2, 2
        ctmrg_tol = 1.0e-9
        Random.seed!(1459)
        ...
        peps0 = adapt(AT, InfinitePEPS(rand, Float64, Pspace, Vspace, Vspace'; unitcell = (Nr, Nc)))
        # make initial state bipartite
        for r in 1:2
            peps0[r + 1, 2] = copy(peps0[r, 1])
        end
        wts0 = SUWeight(peps0)
        ham = adapt(AT, hubbard_model(Float64, Trivial, U1Irrep, InfiniteSquare(Nr, Nc); t = 1.0, U = 6.0, mu = 3.0))
