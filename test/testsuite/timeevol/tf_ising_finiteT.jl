...
function timeevol_ising_finiteT(AT)
    # when g = 2, β = 0.4 and 2β = 0.8 belong to two phases (without and with nonzero σᶻ)
    return @testset "Finite-T SU (force_mpo = $(force_mpo)) ($AT)" for force_mpo in (false, true)
        ham = adapt(AT, transverse_field_ising(Float64, Trivial, InfiniteSquare(Nr, Nc); J = 1.0, g = 2.0))
