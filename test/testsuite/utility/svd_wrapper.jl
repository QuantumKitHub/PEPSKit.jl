...
function utility_svd_wrapper(AT)
    return @testset "svd_wrapper ($AT)" begin
        ...
        r = adapt(AT, randn(dtype, ℂ^m, ℂ^n))
        ...
        symm_r = adapt(AT, randn(dtype, symm_space, symm_space))
        symm_R = adapt(AT, randn(dtype, space(symm_r)))
