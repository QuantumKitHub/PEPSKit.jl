...
function examples_heisenberg(AT)
    return @testset "Heisenberg ($AT)" begin
        ...
        @testset "(1, 1) unit cell AD optimization" begin
            # initialize states
            Random.seed!(123)
            H = adapt(AT, heisenberg_XYZ(InfiniteSquare()))
        ...
        @testset "(1, 2) unit cell AD optimization" begin
            ...
            peps₀ = adapt(AT, InfinitePEPS(ComplexSpace(2), ComplexSpace(Dbond); unitcell))
            env₀, = leading_boundary(CTMRGEnv(peps₀, ComplexSpace(χenv)), peps₀)
        ...
        @testset "Simple update into AD optimization" begin
            ...
            ctmrg_tol = 1.0e-8
            ctmrg_maxiter = 200
            peps = adapt(AT, InfinitePEPS(rand, Float64, Pspace, Vspace; unitcell = (N1, N2)))
            wts = SUWeight(peps)
            normalize!.(peps.A, Inf)

            # Heisenberg model Hamiltonian
            ham = adapt(AT, heisenberg_XYZ(InfiniteSquare(N1, N2); Jx = 1.0, Jy = 1.0, Jz = 1.0))
