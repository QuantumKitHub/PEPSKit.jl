...
function examples_j1j2(AT)
    return @testset "J1J2 ($AT)" begin
        ...
        H = adapt(AT, j1_j2_model(InfiniteSquare(); J2 = 0.25))
        peps₀ = adapt(AT, product_peps(ComplexSpace(2), ComplexSpace(χbond); noise_amp = 1.0e-1))
        ...
        @test E ≈ -0.5618837021945925 atol = 1.0e-3
        @test all(@. isapprox(ξ_h, ξ_ref; atol = 1.0e-1) && isapprox(ξ_v, ξ_ref; atol = 1.0e-1))
        @test ξ_h ≈ ξ_v atol = 1.0e-6  # Test symmetrization of optimized PEPS and environment
    end
end
