...
function examples_pwave(AT)
    return @testset "PWave ($AT)" begin
        # Initialize parameters
        unitcell = (2, 2)
        H = adapt(AT, pwave_superconductor(InfiniteSquare(unitcell...)))
        Dbond = 2
        χenv = 16
        ...
        peps₀ = adapt(AT, InfinitePEPS(Pspace, Vspace, Vspace; unitcell))
