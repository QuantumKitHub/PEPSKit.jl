...
@testset "Cluster bond truncation with projectors" begin
    ...
    for (Vphy, Vns, V) in Vspaces
        ...
        Ms1 = map(1:N) do i
            ...
            return adapt(ROCArray, rand(Vw ⊗ Vphy ⊗ Vns' ⊗ Vns ← Ve))
        end
        normalize!.(Ms1, Inf)
...
@testset "Identity gate on 3-site cluster" begin
    ...
    for (Vphy, Vns, V) in Vspaces
        ...
        Ms1 = map(1:N) do i
            Vw, Ve = Vvirs[i], Vvirs[i + 1]
            return adapt(ROCArray, normalize(rand(Vw ⊗ Vphy ⊗ Vns' ⊗ Vns ← Ve), Inf))
        end
        flips = [isdual(space(M, 1)) for M in Iterators.drop(Ms1, 1)]
        unit = id(Vphy)
    ...
    for (Vphy, Vns, V) in Vspaces
        ...
        Ms1 = map(1:N) do i
            Vw, Ve = Vvirs[i], Vvirs[i + 1]
            return adapt(ROCArray, normalize(rand(Vw ⊗ Vphy ⊗ Vphy' ⊗ Vns' ⊗ Vns ← Ve), Inf))
        end
        flips = [isdual(space(M, 1)) for M in Iterators.drop(Ms1, 1)]
        unit = adapt(ROCArray, id(Vphy))
        gate = reduce(⊗, fill(unit, 3))
        gs = PEPSKit.gate_to_mpo(gate)
        @test mpo_to_gate3(gs) ≈ gate
        for gate_ax in 1:2
...
@testset "Hubbard model SU (MPO gate)" begin
    ...
    peps0 = adapt(ROCArray, InfinitePEPS(rand, Float64, Pspace, Vspace, Vspace'; unitcell = (Nr, Nc)))
    # make initial state bipartite
    for r in 1:2
        peps0[r + 1, 2] = copy(peps0[r, 1])
    end
    wts0 = SUWeight(peps0)
    ham = adapt(ROCArray, hubbard_model(Float64, Trivial, U1Irrep, InfiniteSquare(Nr, Nc); t = 1.0, U = 6.0, mu = 3.0))
