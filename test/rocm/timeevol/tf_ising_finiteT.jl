...
function measure_mag(pepo::InfinitePEPO, env::CTMRGEnv; purified::Bool = false)
    ...
    Mx = adapt(ROCArray, LocalOperator(lattice, ((r, c),) => σˣ(Float64, Trivial)))
    Mz = adapt(ROCArray, LocalOperator(lattice, ((r, c),) => σᶻ(Float64, Trivial)))
    if purified
...
Nr, Nc = 2, 2
ham = adapt(ROCArray, transverse_field_ising(Float64, Trivial, InfiniteSquare(Nr, Nc); J = 1.0, g = 2.0))
pepo0 = adapt(ROCArray, PEPSKit.infinite_temperature_density_matrix(ham))
@test TensorKit.storagetype(ham) <: ROCVector
@test TensorKit.storagetype(pepo0) <: ROCVector
wts0 = SUWeight(pepo0)
...
@testset "Finite-T SU (force_mpo = $(force_mpo))" for force_mpo in (false, true)
    ...
    @test storagetype(bp_env₀) <: ROCArray
