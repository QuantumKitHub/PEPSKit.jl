...
models = [
    adapt(ROCArray, heisenberg_XYZ(InfiniteSquare())),
    adapt(ROCArray, pwave_superconductor(InfiniteSquare())),
]
...
@testset "AD CTMRG energy gradients for $(names[i]) model" verbose = true for i in
    ...
    @testset "ctmrg_alg=:$ctmrg_alg, projector_alg=:$projector_alg, svd_rrule_alg=:$svd_rrule_alg, gradient_alg=(; alg = :$gradient_alg, solver_alg = (; alg = :$gradient_solver_alg))" for (
        ...
        dir = adapt(ROCArray, InfinitePEPS(Pspace, Vspace))
        psi = adapt(ROCArray, InfinitePEPS(Pspace, Vspace))
...
@testset "AD CTMRG energy gradient accuracy regression test (#276)" begin
    ...
    # initialize randomly
    H = adapt(ROCArray, heisenberg_XYZ(InfiniteSquare(1, 1)))
    peps = adapt(ROCArray, PEPSKit.peps_normalize(InfinitePEPS(randn, ComplexF64, physicalspace(H)[1], ComplexSpace(3))))
