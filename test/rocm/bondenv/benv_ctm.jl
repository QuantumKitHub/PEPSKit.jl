...
function get_hubbard_peps(t::Float64 = 1.0, U::Float64 = 8.0)
    H = adapt(ROCArray, hubbard_model(ComplexF64, Trivial, U1Irrep, InfiniteSquare(Nr, Nc); t, U, mu = U / 2))
    Vphy = Vect[FermionParity ⊠ U1Irrep]((0, 0) => 2, (1, 1 // 2) => 1, (1, -1 // 2) => 1)
    peps = adapt(ROCArray, InfinitePEPS(rand, ComplexF64, Vphy, Vphy; unitcell = (Nr, Nc)))
    wts = SUWeight(peps)
    alg = SimpleUpdate(; trunc = trunc_state)
    evolver = TimeEvolver(peps, H, 1.0e-2, 10000, alg, wts)
    peps, = time_evolve(evolver, H; tol = 1.0e-8, verbosity = 1, check_interval = 2000)
...
function get_hubbard_pepo(t::Float64 = 1.0, U::Float64 = 8.0)
    ...
    wts = SUWeight(pepo)
    alg = SimpleUpdate(; trunc = trunc_state, bipartite = false)
    pepo, = time_evolve(pepo, H, 2.0e-3, 500, alg, wts; verbosity = 1, check_interval = 100)
    normalize!.(pepo.A, Inf)
    return pepo
end

function test_benv_ctm(state::Union{InfinitePEPS, InfinitePEPO})
