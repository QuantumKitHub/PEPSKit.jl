...
@testset "Fermionic twists" begin
    ...
    for _ in 1:4 # multiple trials without setting seed
        ...
        A1 = adapt(ROCArray, randn(ComplexF64, Aspace))
        A2 = adapt(ROCArray, randn(ComplexF64, Aspace))
...
@testset "Cost function of LocalTruncation" begin
    ...
    A1 = adapt(ROCArray, normalize(randn(Vphy ⊗ Vphy' ← Vaux ⊗ V ⊗ Vaux' ⊗ Vaux'), Inf))
    A2 = adapt(ROCArray, normalize(randn(Vphy ⊗ Vphy' ← Vaux ⊗ V ⊗ Vaux' ⊗ Vaux'), Inf))
    B1 = adapt(ROCArray, normalize(randn(Vphy ⊗ Vphy' ← Vaux ⊗ Vaux ⊗ Vaux' ⊗ V'), Inf))
    B2 = adapt(ROCArray, normalize(randn(Vphy ⊗ Vphy' ← Vaux ⊗ Vaux ⊗ Vaux' ⊗ V'), Inf))

    P1, P2, info = virtual_projector(A1, A2, B1, B2; trunc = notrunc())
    @test P1 * P2 ≈ adapt(ROCArray, TensorKit.id(domain(P2)))
...
@testset "Virtual space matching" begin
    ...
    ρ = adapt(ROCArray, InfinitePEPO(randn, ComplexF64, Vps, Vns, Ves))
