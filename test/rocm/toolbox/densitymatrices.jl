...
@testset "Single-layer densitymatrix contractions ($I)" for I in keys(ds)
    ...
    env = CTMRGEnv(adapt(ROCArray, ρ_pf), χ)

    O = adapt(ROCArray, rand(d, d))
    @plansor O_pf[W S; N E] := O[p'; p] * ρ[1, 1, 1][p p'; N E S W]

    # Single site
    O_singlesite = adapt(ROCArray, LocalOperator(physicalspace(ρ), ((1, 1),) => O))
    ...
    for inds in zip(
        ...
        O_twosite = adapt(ROCArray, LocalOperator(physicalspace(ρ), inds => O ⊗ O))
...
@testset "Double-layer densitymatrix contractions ($I)" for I in keys(ds)
    ...
    O = adapt(ROCArray, rand(d, d))
    F = adapt(ROCArray, isomorphism(fuse(d ⊗ d'), d ⊗ d'))
    @tensor O_doubled[-1; -2] := F[-1; 1 2] * O[1; 3] * twist(F', 2)[3 2; -2]

    # Single site
    site = (1, 1)
    O_singlesite = adapt(ROCArray, LocalOperator(physicalspace(ρ), (site,) => O))
    E1 = expectation_value(ρ, O_singlesite, ρ, env)
    O_doubled_singlesite = adapt(ROCArray, LocalOperator(physicalspace(ρ_peps), (site,) => O_doubled))
    ...
    for inds in zip(
        ...
        O_doubled_twosite = adapt(ROCArray, LocalOperator(physicalspace(ρ_peps), inds => O_doubled ⊗ O_doubled))
