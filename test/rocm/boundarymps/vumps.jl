...
@testset "(1, 1) PEPS" begin
    ...
    psi = adapt(ROCArray, InfinitePEPS(Vpeps, Vpeps))

    T = adapt(ROCArray, PEPSKit.InfiniteTransferPEPS(psi, 1, 1))
    foreach(V -> (@test V == Vpeps ⊗ Vpeps'), physicalspace(T))
    mps = adapt(ROCArray, initialize_mps(T, [ComplexSpace(20)]))
...
@testset "(2, 2) PEPS" begin
    Vpeps = ComplexSpace(2)
    psi = adapt(ROCArray, InfinitePEPS(Vpeps, Vpeps; unitcell = (2, 2)))
    T = adapt(ROCArray, PEPSKit.MultilineTransferPEPS(psi, 1))
    ...
    mps = adapt(ROCArray, initialize_mps(rand, scalartype(T), T, fill(ComplexSpace(20), 2, 2)))
...
#=@testset "Fermionic PEPS" begin
    ...
    psi = adapt(ROCArray, InfinitePEPS(D, d; unitcell = (1, 1)))
    n = adapt(ROCArray, InfiniteSquareNetwork(psi))
    T = adapt(ROCArray, InfiniteTransferPEPS(psi, 1, 1))
    foreach(V -> (@test V == D ⊗ D'), physicalspace(T))

    # compare boundary MPS contraction to CTMRG contraction
    mps = adapt(ROCArray, initialize_mps(T, [χ]))
    mps, env, ϵ = leading_boundary(mps, T, vumps_alg)
    ...
    n´ = adapt(ROCArray, InfiniteSquareNetwork(map(PEPSKit.mpotensor, PEPSKit.unitcell(n))))
    T´ = adapt(ROCArray, InfiniteMPO(map(PEPSKit.mpotensor, T.O)))
    foreach(V -> (@test V == fuse(D, D')), physicalspace(T´))

    mps´ = adapt(ROCArray, InfiniteMPS(randn, ComplexF64, [physicalspace(T´, 1)], [χ]))
    mps´, env´, ϵ = leading_boundary(mps´, T´, vumps_alg)
    N_vumps´ = abs(prod(expectation_value(mps´, T´)))
...
@testset "PEPO runthrough" begin
    function ising_pepo(beta; unitcell = (1, 1, 1))
        ...
        return adapt(ROCArray, InfinitePEPO(O; unitcell))
    end
    ...
    # single-layer PEPO
    O = ising_pepo(1)
    psi = adapt(ROCArray, PEPSKit.initializePEPS(O, Vpeps))
    T = adapt(ROCArray, InfiniteTransferPEPO(psi, O, 1, 1))
    foreach(V -> (@test V == Vpeps ⊗ Vpepo ⊗ Vpeps'), physicalspace(T))

    mps = adapt(ROCArray, initialize_mps(rand, scalartype(T), T, [ComplexSpace(10)]))
    mps, env, ϵ = leading_boundary(mps, T, vumps_alg)
    f = abs(prod(expectation_value(mps, T)))

    # double-layer PEPO
    O2 = repeat(O, 1, 1, 2)
    psi2 = adapt(ROCArray, initializePEPS(O2, Vpeps))
    T2 = adapt(ROCArray, InfiniteTransferPEPO(psi, O2, 1, 1))
    foreach(V -> (@test V == Vpeps ⊗ Vpepo ⊗ Vpepo ⊗ Vpeps'), physicalspace(T2))

    mps2 = adapt(ROCArray, initialize_mps(rand, scalartype(T2), T2, [ComplexSpace(8)]))
