...
r = adapt(ROCArray, randn(dtype, ℂ^n, ℂ^n))
r = 0.5 * (r + r') # make r Hermitian
R = adapt(ROCArray, randn(space(r)))
R = 0.5 * (R + R')
...
symm_r = adapt(ROCArray, randn(dtype, symm_space, symm_space))
symm_r = 0.5 * (symm_r + symm_r')
symm_R = adapt(ROCArray, randn(dtype, space(symm_r)))
symm_R = 0.5 * (symm_R + symm_R')
