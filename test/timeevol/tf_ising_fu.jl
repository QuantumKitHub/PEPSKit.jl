...
function tfising_fu(g::Float64, maxiter::Int, Dcut::Int, chi::Int; als=true, use_pinv=false)
    ...
    trscheme_env = truncerr(1e-10) & truncdim(chi)
    env = CTMRGEnv(rand, ComplexF64, peps, chi)
    env, = leading_boundary(env, peps; tol=1e-10, verbosity=2, trscheme=trscheme_env)
    ...
    opt_alg = ALSTruncation(; trscheme=trscheme_peps, tol=1e-10, use_pinv=true)
    ...
    time0 = time()
    peps, env, = fu_iter2(ham, peps, env, fu_alg)
