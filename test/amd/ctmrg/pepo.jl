...
function three_dimensional_classical_ising(; beta, J = 1.0)
    ...
    return adapt(ROCArray, TensorMap(o, TMS)), adapt(ROCArray, TensorMap(m, TMS)), adapt(ROCArray, TensorMap(e, TMS))
end
...
@testset "Fixed-point computation for 3D classical ising model" begin
    ...
    # contract
    T = adapt(ROCArray, InfinitePEPO(O; unitcell = (1, 1, 1)))
    psi0 = initializePEPS(T, χpeps)
    n2 = InfiniteSquareNetwork(psi0)
    @test storagetype(n2) <: ROCArray
    env2_0 = CTMRGEnv(n2, χenv)
    n3 = InfiniteSquareNetwork(psi0, T)
    @test storagetype(n3) <: ROCArray
    env3_0 = CTMRGEnv(n3, χenv)
