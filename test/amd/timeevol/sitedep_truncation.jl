...
Random.seed!(48736)
states = (
          adapt(ROCArray, InfinitePEPS(randn, elt, Vps, Vns, Ves1)),
          adapt(ROCArray, InfinitePEPO(randn, elt, Vps, Vns, Ves2)),
)
...
@testset "Simple update on $(typeof(state0).name.wrapper), bipartite = $(bipartite)" for
    ...
    ham = adapt(ROCArray, j1_j2_model(elt, U1Irrep, InfiniteSquare(Nr, Nc); J1 = 1.0, J2, sublattice = false))
